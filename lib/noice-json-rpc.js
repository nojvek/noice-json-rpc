...
class Client extends events_1.EventEmitter {
    constructor(socket, opts) {
        super();
        this._pendingMessageMap = new Map();
        ...
        return new Proxy({}, {
            get: (target, prop) => {
                ...
                if (prop == "__proto__") {
                    ...
                    target[prop] = (handler) => this.on(`${domain}.${method}`, handler);
                }
                else if (prop.substr(0, 4) === "emit" && prop.length > 5) {
                    const method = prop[4].toLowerCase() + prop.substr(5);
                    target[prop] = (params) => this.notify(`${domain}.${method}`, params);
                }
                else {
                    const method = prop;
                    target[prop] = (params) => this.call(`${domain}.${method}`, params);
                }
                return target[prop];
            }
        });
...
class Server extends events_1.EventEmitter {
    ...
    processMessage(messageStr, socket) {
        ...
        try {
            ...
            return this._sendError(socket, message, -32700 /* ParseError */);
        }
        ...
        if (message && message.method && typeof message.method == "string") {
            if (message.id && typeof message.id === "number") {
                ...
                if (handler) {
                    try {
                        const result = handler.call(null, message.params);
                        if (result instanceof Promise) {
                            ...
                            result.then((actualResult) => {
                                ...
                                this._sendError(socket, message, -32603 /* InternalError */, error);
                            });
                        }
                        else {
                    ...
                    this._sendError(socket, message, -32601 /* MethodNotFound */);
                }
            }
            else {
            ...
            this._sendError(socket, message, -32600 /* InvalidRequest */);
        }
    }
    /** Set logging for all received and sent messages */
    setLogging({ logEmit, logConsole } = {}) {
    ...
    _sendError(socket, request, errorCode, errorData) {
        this._send(socket, {
        ...
        return new Proxy({}, {
            get: (target, prop) => {
                ...
                if (prop == "__proto__") {
                    ...
                    target[prop] = (module) => {
                        if (!module)
                            throw new Error("An iterable object expected to expose functions");
